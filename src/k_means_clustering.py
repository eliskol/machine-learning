class KMeans:
    ...
    def find_centers(self):
        ...
        centers = {
            cluster_index: [0 for i in range(len(self.data[0]))]
        ...
        for index, cluster in self.clusters.items():
            ...
            for point_index in cluster:
                for coord_index in range(length_of_datapoints):
                    centers[index][coord_index] += (
                        self.data[point_index][coord_index] / num_of_points_in_cluster
                    )
    ...
    def reassign_clusters(self):
        ...
        for point_index in range(len(self.data)):
            distances = {
                cluster_index: self.find_distance_from_center(
