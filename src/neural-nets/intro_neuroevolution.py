...
def next_generation(neural_nets):
    ...
    for i in range(15):
        neural_net_RSS_list = [neural_net.compute_RSS() for neural_net in neural_nets]
        assert neural_nets[
...
beginning = time.time()
data = [beginning, 0, sum(neural_net.compute_RSS() for neural_net in neural_nets) / 15]
...
for i in range(5000):
    ...
    data = [
        time.time(),
        i,
        sum(neural_net.compute_RSS() for neural_net in neural_nets) / 15,
    ]
    print(
        "avg RSS at generation",
        i + 1,
        sum(neural_net.compute_RSS() for neural_net in neural_nets) / 15,
        "change in avg rss:",
        (sum(neural_net.compute_RSS() for neural_net in neural_nets) / 15) - prev_RSS,
    )
    print(
        "avg mutation rate",
        sum(neural_net.mutation_rate for neural_net in neural_nets) / 15,
    )
